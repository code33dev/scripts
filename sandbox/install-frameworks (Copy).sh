...
cd /mnt/development/code33/ && cd src/framework && rm -rf build && mkdir build && cd build && cmake -DCMAKE_INSTALL_PREFIX=/mnt/development/sdk .. && make && make install
cd /mnt/development/sdk
echo 'Compressing this fucking shit!!'
timestamp=$(date +%Y-%m-%d_%H-%M-%S) && tar -cJf sdk_code33_gcc$1_$timestamp.tar.xz .
